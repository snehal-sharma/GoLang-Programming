...
func HeightofBinaryTreeRecursive(root *BinaryTreeNode) int {
	if root == nil {
		return 0
	}
...
func HeightOfBinaryTree(root *BinaryTreeNode) int {
	myQueue := Queue{Front: -1, Rear: -1, Capacity: 14}
	myQueue.CreateQueue()
	if root == nil {
		return 0
	}
...
func main() {
	...
	height := HeightOfBinaryTree(&binaryTree)
	fmt.Println("Height of Binary Tree :", height)
}
