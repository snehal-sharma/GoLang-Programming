...
* Sending to an unbuffered channel without recieving it causes a goroutine leak and subsequent deadlock. Receiving from a channel without sending anything in returns a zero value. Sending/Receiving on a nil channel blocks forever(deadlock). Closing a nil channel causes panic.
